...
#include "tensorflow/contrib/lite/c/tensor_jni.h"
#include <cstring>
#include <memory>
...
size_t writeOneDimensionalArray(JNIEnv* env, jobject object, TfLiteType type,
  ...
  if (to_copy > dst_size) {
    throwException(/*env*/NULL, EXCEPTION_TYPE::IllegalStateException,
...
size_t readOneDimensionalArray(JNIEnv* env, TfLiteType data_type,
  ...
  if (size > src_size) {
    throwException(
        /*env*/NULL, EXCEPTION_TYPE::IllegalStateException,
  ...
  switch (data_type) {
    ...
    default: {
      throwException(/*env*/NULL, EXCEPTION_TYPE::IllegalStateException, "invalid DataType(%d)",
                     data_type);
    }
  }
  return 0;
