...
#include "tensorflow/contrib/lite/c/nativeinterpreterwrapper_jni.h"
...
tflite::Interpreter* convertLongToInterpreter(JNIEnv* env, jlong handle) {
  if (handle == 0) {
    throwException(/*env*/NULL, EXCEPTION_TYPE::IllegalArgumentException,
...
tflite::FlatBufferModel* convertLongToModel(JNIEnv* env, jlong handle) {
  if (handle == 0) {
    throwException(/*env*/NULL, EXCEPTION_TYPE::IllegalArgumentException, "Invalid handle to model.");
    return nullptr;
  }
...
Java_org_tensorflow_lite_NativeInterpreterWrapper_getInputNames(JNIEnv* env,
  ...
  if (string_class == nullptr) {
    throwException(/*env*/NULL, EXCEPTION_TYPE::UnsupportedOperationException,
...
Java_org_tensorflow_lite_NativeInterpreterWrapper_createErrorReporter(
  ...
  BufferErrorReporter* error_reporter =
      new BufferErrorReporter(/*env*/NULL, static_cast<int>(size));
...
Java_org_tensorflow_lite_NativeInterpreterWrapper_run(
  ...
  if (status != kTfLiteOk) {
    throwException(/*env*/NULL, EXCEPTION_TYPE::NullPointerException, "Can not resize the input: %s",
                   error_reporter->CachedErrorMessage());
    return nullptr;
  }
  // allocates memory
  ...
  if (status != kTfLiteOk) {
    throwException(/*env*/NULL, EXCEPTION_TYPE::NullPointerException,
